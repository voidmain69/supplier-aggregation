...
The domain defines the :class:`~matching.domain.embedding.Embedder` protocol; this adapter
...
``MATCHING_EMBEDDER_URL``: when that is unset the service falls back to the dependency-free
:class:`~matching.domain.embedding.HashingEmbedder`, so tests and offline runs need no GPU. Both
...
from matching.domain.embedding import EMBEDDING_DIM, Embedder, HashingEmbedder
