...
from search.adapters.canonical_repository import index_canonical_document
...
def build_canonical_updated_handler(
    session_factory: async_sessionmaker[AsyncSession],
    *,
    embedder: Embedder | None = None,
    sparse_embedder: SparseEmbedder | None = None,
) -> EventHandler:
    """Build a handler for ``catalog.product.updated`` that indexes the canonical product card.

    Same embedding seam as the supplier handler — a canonical card is embedded from its title+brand
    so hybrid search can return canonical product ids.
    """
    embed = embedder or HashingEmbedder()

    async def handle(envelope: dict[str, Any]) -> None:
        event_id = envelope["id"]
        payload = CatalogProductUpdated(**envelope["data"])
        text = product_text(payload.title, payload.brand)
        vector = embed.embed(text)
        sparse = sparse_embedder.embed_sparse(text) if sparse_embedder is not None else None
        async with session_factory() as session, session.begin():
            if await session.get(ProcessedEvent, event_id) is not None:
                return  # already indexed — idempotent skip
            await index_canonical_document(session, payload, embedding=vector, sparse=sparse)
            session.add(ProcessedEvent(event_id=event_id))
