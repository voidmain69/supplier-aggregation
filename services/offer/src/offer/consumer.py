...
Consumes ``supplier.offer.price-changed`` and upserts offers idempotently.
Run with ``python -m offer.consumer``.
"""
...
async def run() -> None:
    ...
    async with KafkaPublisher(settings.kafka_bootstrap, client_id="offer-dlq") as dlq:
