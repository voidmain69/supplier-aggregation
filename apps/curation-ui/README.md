...
рішення confirm/reject з аудитом, перегляд канонічного каталогу. Ходить **тільки** через
...
(`API_GATEWAY_CORS_ALLOW_ORIGINS`) і bearer-токен зі скоупом `matching:curate` (плюс `catalog:read`,
`offers:read`, `prices:read` для повного контексту). У `make up` усе піднімається разом.
...
src/
  ...
  pages/      сторінки-композиції: login · queue · review · canonical
  features/   сценарії: curation-decision · commercial-context · token-auth
  entities/   доменні відображення: supplier-product · canonical-product · curation · offer ·
              price-history · attribute-diff
