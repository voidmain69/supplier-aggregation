...
- **OIDC для операторів + `curation-ui` v2 (решта)**: зараз статичний bearer; **журнал рішень уже
  доставлено** (append-only аудит-лог matching + сторінка в curation-ui). Лишаються: категорійний
  мапінг, моніторинг синків, дашборд оператора (див. [docs/curation-ui-plan.md](docs/curation-ui-plan.md) §v2).
